...
def intent_exists(intents, tag, patterns):
    for intent in intents:
        if not isinstance(intent, dict):
            print(f"Unexpected intent format: {intent}")
            continue
...
def parse_pdf_and_extract_intents(pdf_file, intents_json_file):
    try:
        with open(intents_json_file, 'r') as file:
            ...
            if not isinstance(intents, list):
                raise ValueError(f"Intents file should be a list, got {type(intents).__name__} instead.")
    except (FileNotFoundError, json.JSONDecodeError, ValueError) as e:
        print(f"Error loading intents: {e}")
        intents = []  # Initialize as empty list if file doesn't exist or is corrupted
    ...
    print(f"Extracted {len(new_intents)} new intents. Saved to {intents_json_file}.")

# Example usage
if __name__ == "__main__":
    pdf_file = str(input("Enter name of pdf:"))  # Replace with your PDF file path
